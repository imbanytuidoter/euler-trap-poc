...
abstract contract TrapHarness is Test {
    ...
    function _flushLogsToTrap() internal {
        ...
        for (uint256 i = 0; i < recorded.length; i++) {
            toFeed[i] = EventLog({topics: recorded[i].topics, data: recorded[i].data, emitter: recorded[i].emitter});
        }
    ...
    function _baselineWithReserves(uint256 reserves, uint256 borrows) internal view returns (bytes memory) {
        return abi.encode(
            EulerFinanceTrap.CollectOutput({
    ...
    function _windowWithCurrentAndBase(bytes memory current, bytes memory base) internal view returns (bytes[] memory) {
        bytes[] memory w = new bytes[](5);
        w[0] = current;
        w[1] = _cleanBaselineEncoded();
        w[2] = _cleanBaselineEncoded();
